...
// The below test asserts a "Promise" based network returns a value we expect.
    ...
    describe('#getAirportStatus', function() {
        context('with a valid airport code', function() {
            it('returns matching airport code', function() {
                airport_code = 'SEA';
                // The requestAirportStatus method will be an eventual method that 
                // makes request to the airport info service.
                var value = subject.requestAirportStatus(airport_code).then(function(obj) {
                    return obj.IATA;
                });
                // The eventually portion of the matcher allows to make
                // Assertions about the data a Promise returns without
                // Requiring the use of callbacks
                return expect(value).to.eventually.eq(airport_code);
            });
        });
        // Test case that keeps check of invalid airport codes provided.
